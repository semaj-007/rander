...
The app's `debug` build type already targets `https://10.0.2.2:5001/` (10.0.2.2 is
...
REQ-AUTH-1 calls for real SSO via OpenID Connect against the university's Microsoft
Azure AD tenant. Standing that up needs an actual Azure AD App Registration, which
isn't something to fake convincingly - so for the prototype, **`AuthController.DevLogin`**
stands in for it: it creates (or finds) a `User` row from an email + name and hands
back a JWT signed with the `Jwt:Key` in `appsettings.Development.json`. Every other
endpoint requires that JWT via `Authorization: Bearer <token>` (REQ-API-1).

**Before any real deployment:** delete `AuthController`, and point the JWT bearer
options in `Program.cs` at Azure AD's authority/audience instead of the local
signing key.
